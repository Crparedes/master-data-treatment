cMEPlot <- function(obj, ...){
  UseMethod("cMEPlot")
}
cMEPlot.design <- function(obj, ..., response=NULL){
  ...
  cMEPlot(lm(obj, degree=1, response=response), ...)
}
cMEPlot.default <-
  function(obj, main=paste("Main effects plot for", respnam), pch=15, 
    ...
    for (i in 1:nmain){
      ...
      if (i==1)
        plot(c(-1,1),predmat[,i],main=terms1[i],xlab="",xpd=NA, ylab='Eficiencia (%)',type="b",
             xlim=c(-1.3,1.3),ylim=c(min(ax),max(ax)), axes=FALSE, cex=2, 
             cex.lab=1.5*cex.yax, cex.axis=1.5, pch=pch, mgp=c(mgp.ylab,1,0), 
             cex.main=cex.main, lwd=lwd)
      else {if (i %in% 5:6) xlim = c(1.3,-1.3) else xlim=c(-1.3,1.3)
        plot(c(-1,1),predmat[,i],main=terms1[i],xlab="",ylab="",type="b",
                xlim=xlim,ylim=c(min(ax),max(ax)), axes=FALSE, cex=2, 
