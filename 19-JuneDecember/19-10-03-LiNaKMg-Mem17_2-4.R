...
sepFactor[[1]]
NewSepFac <- list(sepfactor(main = TransFrac[[1]], secon = TransFrac[[2]]))
...
for (i in 1:3) {
  p <- transPlotWR(trans = list(TransFrac[[4*i-3]], TransFrac[[4*i-1]]), trend = list(TransNLS[[1]]),
                   ...
                   bw = TRUE, srs = 0.55, xlab = 'Tiempo (h)')
