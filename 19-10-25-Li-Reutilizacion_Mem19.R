...
for (i in 1:length(AliAbs)) {
  if(i %in% c(1:12, 21:32)) {
    ...
    AliConc[[i]] <- signal2conc(signal = AliAbs[[i]], model = CalModels$Lithium.2)
  }
}
#-----CONCENTRACIONES A FRACCIONES-------------------------------------------
TransFrac <- vector(mode = "list", length = length(AliAbs) / 2)
names(TransFrac) <- names(AliAbs)[seq(1, 40, 2)]
for (i in 1:20) {
...
p <- multiPlotSP(trans = FracAve, phase = 'Strip', trend = TransNLSAve, legend = FALSE,
                 plot = FALSE, nat.color = FALSE, size = 2.8, shape = 16)
...
final <- vector()
for (i in 1:10) final <- c(final, FracAve[[i]]$Fraction[10])
...
p1 <- transPlotWR(trans = list(TransFrac[[1]], TransFrac[[11]]), bw = TRUE, srs = 0.55) +
