  ...
  if (explicit) {
    ...
    if (!missing(trend)) {
      if (trend[[1]]$model == 'paredes') {
        ...
        for (i in 1:length(trend)) {
          p <- p + stat_function(fun = transmem:::AddParTrend(trend, i, 'strip', e),
                                 args = list(i = i), color = "red",
                                 xlim = xlimTrendWR(1, trans))
          p <- p + stat_function(fun = transmem:::AddParTrend(trend, i, 'feed', e),
    ...
    p <- ggplot(data = mtrans, aes(x = Time, y = Fraction, group = Phase)) +
      ...
      theme(panel.grid.major = element_blank(),
            ...
            axis.text.x = element_text(color = "black"),
            axis.text.y = element_text(color = "black")) +
      stat_function(fun = transmem:::AddParTrend(mtrend, 1, 'strip', eccen),
                    color = colbw[1], args = (i = 1),
                    xlim = c(0, mtrans$Time[length(mtrans$Time)])) +
      stat_function(fun = transmem:::AddParTrend(mtrend, 1, 'feed', eccen),
    ...
    if (!missing(secondary)) {
      msecon <- transmem:::transColapse(trans = secondary)
    ...
    if (!missing(tertiary)) {
      mterna <- transmem:::transColapse(trans = tertiary)
